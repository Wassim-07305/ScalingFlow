...
CREATE INDEX idx_business_scores_user_id ON public.business_scores(user_id);
CREATE INDEX idx_business_scores_created_at ON public.business_scores(user_id, created_at DESC);

ALTER TABLE public.business_scores ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view own business scores"
  ON public.business_scores FOR SELECT
  USING (auth.uid() = user_id);

CREATE POLICY "Users can insert own business scores"
