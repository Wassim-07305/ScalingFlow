...
async function processAffiliateCommission(
  ...
  // Récupérer le referred_by du profil
  const { data: profile } = await supabase
    .from("profiles")
    .select("referred_by")
    .eq("id", userId)
  ...
  const { data: affiliate } = await supabase
    .from("affiliates")
    .select(
      `id, user_id, custom_commission_rate, total_earned, total_conversions,
       affiliate_programs!inner(
         id, commission_type, commission_rate, recurring_months, is_active
       )`,
    )
    .eq("affiliate_code", profile.referred_by)
    .eq("status", "active")
  ...
  if (!affiliate?.affiliate_programs) return;
  ...
  // Compter les commissions existantes (pour vérifier recurring_months)
  let existingCount = 0;
  if (isRecurring) {
    const { count } = await supabase
      .from("commissions")
  ...
  const result = calculateCommission({
    sourceAmount: amount,
    commissionType: program.commission_type as "one_time" | "recurring" | "tiered",
  ...
  // Marquer le referral comme converti (1ère fois seulement)
  ...
  await supabase
    .from("affiliates")
    .update({
      total_earned: (affiliate.total_earned ?? 0) + result.amount,
      total_conversions: isFirstConversion
        ? (affiliate.total_conversions ?? 0) + 1
        : affiliate.total_conversions,
    })
    .eq("id", affiliate.id);

  // XP + badges pour l'affilié (non-bloquant)
  awardXPForUser(affiliate.user_id, "affiliate.conversion").catch(() => {});

  // Email affilié "Commission générée" (non-bloquant)
  if (resend) {
    const { data: affiliateProfile } = await supabase
      .from("profiles")
      .select("full_name, email")
