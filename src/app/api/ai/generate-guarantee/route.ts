...
import { guaranteeGeneratorPrompt, type GuaranteeContext, type GuaranteeResult } from "@/lib/ai/prompts/guarantee-generator";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const rl = await rateLimit(user.id, "generate-guarantee", { limit: 5, windowSeconds: 60 });
    if (!rl.allowed) {
