  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-semibold text-text-primary">Social Assets</h3>
          ...
          <div className="grid gap-3 md:grid-cols-3">
            {data.testimonial_cards.map((card, i) => (
              <div key={i} className="rounded-xl border border-border-default bg-bg-tertiary p-4 space-y-2">
                ...
                <p className="text-sm text-text-primary italic">&ldquo;{card.quote}&rdquo;</p>
                <div className="flex items-center justify-between">
                  <span className="text-xs text-text-secondary">{card.client_name} — {card.client_role}</span>
                  <span className="text-[10px] text-text-muted">{card.format}</span>
                </div>
              </div>
            ))}
          </div>
            ...
            {data.social_banners.map((banner, i) => (
              <div key={i} className="rounded-xl border border-border-default bg-bg-tertiary p-4">
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-[10px] px-2 py-0.5 rounded-full bg-accent/10 text-accent font-medium">{banner.platform}</span>
                  <span className="text-[10px] text-text-muted">{banner.type} — {banner.dimensions}</span>
                </div>
                <p className="text-sm font-semibold text-text-primary">{banner.headline}</p>
                <p className="text-xs text-text-secondary mt-1">{banner.subline}</p>
                <p className="text-xs text-accent mt-1">{banner.cta}</p>
                <p className="text-[10px] text-text-muted mt-2 italic">{banner.design_notes}</p>
              </div>
            ))}
          </div>
        </section>
      ...
      {data.highlight_covers && data.highlight_covers.length > 0 && (
        <section>
          <h4 className="text-sm font-medium text-accent mb-3">Highlight Covers</h4>
          <div className="flex gap-3 overflow-x-auto pb-2">
            {data.highlight_covers.map((h, i) => (
              <div key={i} className="flex flex-col items-center gap-1.5 min-w-[80px]">
                ...
                <span className="text-xs text-text-primary font-medium">{h.name}</span>
              </div>
            ))}
          </div>
        </section>
      ...
      {data.email_signature && (
        <section>
          <h4 className="text-sm font-medium text-accent mb-3">Signature Email</h4>
          <div className="rounded-xl border border-border-default bg-bg-tertiary p-4 max-w-md">
            <p className="text-sm font-semibold text-text-primary">{data.email_signature.name}</p>
            <p className="text-xs text-text-secondary">{data.email_signature.title}</p>
            <p className="text-xs text-accent mt-1 italic">{data.email_signature.tagline}</p>
            <p className="text-xs text-text-muted mt-2">{data.email_signature.cta_text}</p>
          </div>
        </section>
      )}
      ...
      {data.social_proof_badges && data.social_proof_badges.length > 0 && (
        <section>
          <h4 className="text-sm font-medium text-accent mb-3">Badges Preuve Sociale</h4>
          <div className="flex flex-wrap gap-2">
            {data.social_proof_badges.map((badge, i) => (
              <div key={i} className="px-3 py-2 rounded-lg border border-accent/20 bg-accent/5">
                <p className="text-xs font-medium text-accent">{badge.text}</p>
                <p className="text-[10px] text-text-muted mt-0.5">{badge.usage}</p>
              </div>
            ))}
          </div>
        </section>
