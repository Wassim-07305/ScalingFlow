...
import { TrendingUp, Eye, MousePointer, DollarSign, Loader2, Megaphone } from "lucide-react";
...
function getStatusVariant(status: AdCampaign["status"]): "cyan" | "muted" | "yellow" | "default" {
  switch (status) {
    case "active":
      return "cyan";
    case "paused":
        ...
        <CardContent>
          <div className="space-y-3">
            {campaigns.map((campaign) => {
              const budget = campaign.total_budget ?? campaign.daily_budget ?? 0;
              const ctr =
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-text-muted">
                        Budget: {budget}€ &middot; Dépensé: {campaign.total_spend}€
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-6 text-right">
                    <div>
                      ...
                      <p className="text-sm font-medium text-info">{ctr.toFixed(2)}%</p>
                    </div>
                    <div>
                      <p className="text-xs text-text-muted">CPA</p>
                      <p className="text-sm font-medium text-accent">{cpa.toFixed(2)}€</p>
                    </div>
                    <div>
                      <p className="text-xs text-text-muted">Conv.</p>
                      <p className="text-sm font-medium text-accent">{campaign.total_conversions}</p>
                    </div>
                  </div>
                </div>
              );
