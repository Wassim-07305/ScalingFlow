...
const MOCK_DECISIONS: Decision[] = [
  ...
  },
  {
    ...
    reason: "Retargeting sous-performant — réallocation depuis la campagne la plus performante",
    status: "en_attente",
    details: "Réalloquer 30€ du budget Before/After Static vers Témoignage Client A",
  },
  {
    id: "dec_004",
    timestamp: "2026-03-15T14:30:00",
...
const MOCK_CYCLES: CreativeCycle[] = [
  { week: 12, startDate: "2026-03-16", endDate: "2026-03-22", winnersCount: 0, losersCount: 0, newCreatives: 5, status: "planifie" },
  { week: 11, startDate: "2026-03-09", endDate: "2026-03-15", winnersCount: 2, losersCount: 2, newCreatives: 4, status: "en_cours" },
  { week: 10, startDate: "2026-03-02", endDate: "2026-03-08", winnersCount: 3, losersCount: 1, newCreatives: 6, status: "termine" },
  { week: 9, startDate: "2026-02-23", endDate: "2026-03-01", winnersCount: 2, losersCount: 3, newCreatives: 5, status: "termine" },
];

const MOCK_WINNER_PATTERNS: WinnerPattern[] = [
  { element: "Hook émotionnel (douleur/frustration)", category: "Hook", score: 92 },
  { element: "Témoignage client réel", category: "Format", score: 88 },
  { element: "Chiffres concrets dans le titre", category: "Copywriting", score: 85 },
  { element: "CTA urgence (places limitées)", category: "CTA", score: 82 },
  { element: "Format vidéo < 30 secondes", category: "Format", score: 78 },
  { element: "Angle transformation (avant/après)", category: "Angle", score: 75 },
];

const SCALING_TIERS: ScalingTier[] = [
  { id: 1, label: "Palier 1 — Démarrage", budgetMin: 0, budgetMax: 50, scalePercent: 20, roasRequired: 2.0, active: true },
  { id: 2, label: "Palier 2 — Croissance", budgetMin: 50, budgetMax: 200, scalePercent: 15, roasRequired: 2.5, active: false },
  { id: 3, label: "Palier 3 — Accélération", budgetMin: 200, budgetMax: 500, scalePercent: 10, roasRequired: 3.0, active: false },
  { id: 4, label: "Palier 4 — Volume", budgetMin: 500, budgetMax: 99999, scalePercent: 5, roasRequired: 3.5, active: false },
];
...
function getStatusConfig(status: CreativeKPI["status"]) {
  switch (status) {
    case "performant":
      return { label: "Performant", color: "text-emerald-400", bg: "bg-emerald-500/10", dot: "bg-emerald-400" };
    case "a_surveiller":
      return { label: "À surveiller", color: "text-yellow-400", bg: "bg-yellow-500/10", dot: "bg-yellow-400" };
    case "sous_performant":
      return { label: "Sous-performant", color: "text-red-400", bg: "bg-red-500/10", dot: "bg-red-400" };
  }
}

function getActionIcon(type: Decision["actionType"]) {
  switch (type) {
    case "couper": return Scissors;
    case "scaler": return TrendingUp;
    case "realloquer": return ArrowUpRight;
    case "fatigue": return RefreshCw;
...
function getActionLabel(type: Decision["actionType"]) {
  switch (type) {
    case "couper": return "Couper";
    case "scaler": return "Scaler";
    case "realloquer": return "Réalloquer";
    case "fatigue": return "Fatigue";
    case "rollback": return "Rollback";
  }
}

function getActionBadgeVariant(type: Decision["actionType"]): "red" | "default" | "blue" | "yellow" | "purple" {
  switch (type) {
    case "couper": return "red";
    case "scaler": return "default";
    case "realloquer": return "blue";
...
function getDecisionStatusLabel(status: Decision["status"]) {
  switch (status) {
    case "applique": return "Appliqué";
    case "en_attente": return "En attente";
    case "annule": return "Annulé";
  }
}

function getDecisionStatusBadge(status: Decision["status"]): "default" | "yellow" | "muted" {
  switch (status) {
    case "applique": return "default";
    case "en_attente": return "yellow";
    case "annule": return "muted";
  }
}
...
function MonitoringTab() {
  ...
  const thresholds = useMemo(() => {
    try {
      ...
      return saved ? JSON.parse(saved) as ThresholdConfig : DEFAULT_THRESHOLDS;
    } catch {
  ...
  const summary = useMemo(() => {
    const performant = creatives.filter((c) => c.status === "performant").length;
    const surveiller = creatives.filter((c) => c.status === "a_surveiller").length;
    const sousPerformant = creatives.filter((c) => c.status === "sous_performant").length;
    const totalSpend = creatives.reduce((s, c) => s + c.spend, 0);
    const totalConversions = creatives.reduce((s, c) => s + c.conversions, 0);
    const avgRoas = creatives.reduce((s, c) => s + c.roas, 0) / (creatives.length || 1);
    return { performant, surveiller, sousPerformant, totalSpend, totalConversions, avgRoas };
  }, [creatives]);
  ...
  const handleAddCreative = () => {
    ...
    if (cpa > thresholds.cpaMax || roas < thresholds.roasMin || ctr < thresholds.ctrMin) {
      status = "sous_performant";
    } else if (cpa > thresholds.cpaMax * 0.8 || roas < thresholds.roasMin * 1.2 || frequency > thresholds.frequenceMax * 0.8) {
      status = "a_surveiller";
    }
    ...
    setFormData({ name: "", campaignName: "", cpm: "", ctr: "", cpc: "", cpa: "", roas: "", spend: "", conversions: "", impressions: "", frequency: "" });
    setShowForm(false);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        {[
          { label: "Performantes", value: summary.performant, color: "text-emerald-400", bg: "bg-emerald-500/10" },
          { label: "À surveiller", value: summary.surveiller, color: "text-yellow-400", bg: "bg-yellow-500/10" },
          { label: "Sous-perf.", value: summary.sousPerformant, color: "text-red-400", bg: "bg-red-500/10" },
          { label: "Dépense tot.", value: `${summary.totalSpend.toFixed(0)}€`, color: "text-blue-400", bg: "bg-blue-500/10" },
          { label: "Conversions", value: summary.totalConversions, color: "text-purple-400", bg: "bg-purple-500/10" },
          { label: "ROAS moy.", value: `${summary.avgRoas.toFixed(2)}x`, color: "text-emerald-400", bg: "bg-emerald-500/10" },
        ].map((item) => (
          <div key={item.label} className={cn("p-3 rounded-xl border border-border-default", item.bg)}>
          ...
          <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
              <div className="col-span-2">
                <Label className="text-xs text-text-muted">Nom de la creative</Label>
                <Input value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} placeholder="Ex: Hook Douleur v4" />
              </div>
              <div className="col-span-2">
                <Label className="text-xs text-text-muted">Campagne</Label>
                <Input value={formData.campaignName} onChange={(e) => setFormData({ ...formData, campaignName: e.target.value })} placeholder="Ex: Acquisition Cold" />
              </div>
              {([
                ...
                { key: "spend" as const, label: "Dépense (€)", placeholder: "250" },
                { key: "conversions" as const, label: "Conversions", placeholder: "12" },
                { key: "impressions" as const, label: "Impressions", placeholder: "30000" },
                { key: "frequency" as const, label: "Fréquence", placeholder: "1.5" },
              ]).map((field) => (
                <div key={field.key}>
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
            <div className="p-2.5 rounded-lg bg-bg-tertiary border border-border-default">
              ...
              <p className="font-semibold text-text-primary">{thresholds.ctrMin}%</p>
              <p className="text-[10px] text-red-400">{"< seuil = rouge"}</p>
            </div>
            <div className="p-2.5 rounded-lg bg-bg-tertiary border border-border-default">
              <span className="text-text-muted">CPA max</span>
              <p className="font-semibold text-text-primary">{thresholds.cpaMax}€</p>
              <p className="text-[10px] text-red-400">{"> seuil = rouge"}</p>
            </div>
            <div className="p-2.5 rounded-lg bg-bg-tertiary border border-border-default">
              <span className="text-text-muted">ROAS min</span>
              <p className="font-semibold text-text-primary">{thresholds.roasMin}x</p>
              <p className="text-[10px] text-red-400">{"< seuil = rouge"}</p>
            </div>
            <div className="p-2.5 rounded-lg bg-bg-tertiary border border-border-default">
              <span className="text-text-muted">Fréquence max</span>
              <p className="font-semibold text-text-primary">{thresholds.frequenceMax}</p>
              <p className="text-[10px] text-yellow-400">{"> seuil = fatigue"}</p>
            </div>
          </div>
        </CardContent>
      </Card>
                ...
                >
                  ...
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-semibold text-text-primary">{creative.name}</p>
                        <p className="text-[10px] text-text-muted">{creative.campaignName}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      <Badge variant={creative.status === "performant" ? "default" : creative.status === "a_surveiller" ? "yellow" : "red"}>
                        {cfg.label}
                      </Badge>
                      {creative.trend === "up" && <TrendingUp className="h-3.5 w-3.5 text-emerald-400" />}
                      {creative.trend === "down" && <TrendingDown className="h-3.5 w-3.5 text-red-400" />}
                      {creative.trend === "stable" && <Activity className="h-3.5 w-3.5 text-text-muted" />}
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-4 md:grid-cols-7 gap-2 mb-2">
                    {[
                      { label: "CPM", value: `${creative.cpm.toFixed(2)}€`, warn: creative.cpm > 12 },
                      { label: "CTR", value: `${creative.ctr.toFixed(2)}%`, warn: creative.ctr < thresholds.ctrMin },
                      { label: "CPC", value: `${creative.cpc.toFixed(2)}€`, warn: creative.cpc > 1.5 },
                      { label: "CPA", value: `${creative.cpa.toFixed(0)}€`, warn: creative.cpa > thresholds.cpaMax },
                      { label: "ROAS", value: `${creative.roas.toFixed(1)}x`, warn: creative.roas < thresholds.roasMin },
                      { label: "Dépense", value: `${creative.spend.toFixed(0)}€`, warn: false },
                      { label: "Conv.", value: `${creative.conversions}`, warn: false },
                    ].map((kpi) => (
                      <div key={kpi.label} className="text-center">
                        <p className="text-[9px] text-text-muted uppercase">{kpi.label}</p>
                        <p className={cn("text-sm font-semibold", kpi.warn ? "text-red-400" : "text-text-primary")}>
                          {kpi.value}
                        </p>
                      </div>
                    ))}
...
function DecisionsAutoTab() {
  ...
  const handleApplyAll = () => {
    setDecisions((prev) =>
      prev.map((d) => (d.status === "en_attente" ? { ...d, status: "applique" as const } : d))
    );
  ...
  const handleApplyDecision = (id: string) => {
    setDecisions((prev) =>
      prev.map((d) => (d.id === id ? { ...d, status: "applique" as const } : d))
    );
    toast.success("Décision appliquée");
  };

  const pendingCount = decisions.filter((d) => d.status === "en_attente").length;
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2 text-sm">
            ...
            <span className="text-text-secondary">{pendingCount} décision{pendingCount !== 1 ? "s" : ""} en attente</span>
          </div>
          <div className="flex items-center gap-2 text-sm">
            <div className="w-2 h-2 rounded-full bg-emerald-400" />
            <span className="text-text-secondary">{decisions.filter((d) => d.status === "applique").length} appliquée{decisions.filter((d) => d.status === "applique").length !== 1 ? "s" : ""}</span>
          </div>
        </div>
        <div className="flex gap-2">
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-4">
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setThresholds({ ...thresholds, cpaMax: parseFloat(e.target.value) || 0 })}
                />
              </div>
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setThresholds({ ...thresholds, roasMin: parseFloat(e.target.value) || 0 })}
                />
              </div>
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setThresholds({ ...thresholds, ctrMin: parseFloat(e.target.value) || 0 })}
                />
              </div>
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setThresholds({ ...thresholds, frequenceMax: parseFloat(e.target.value) || 0 })}
                />
              </div>
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setThresholds({ ...thresholds, budgetScaleIncrement: parseFloat(e.target.value) || 0 })}
                />
              </div>
            </div>
            <Button size="sm" onClick={() => { saveThresholds(thresholds); setShowConfig(false); }}>
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-start gap-3 flex-1">
                      <div className={cn(
                        ...
                        decision.actionType === "couper" ? "bg-red-500/10" :
                        decision.actionType === "scaler" ? "bg-emerald-500/10" :
                        decision.actionType === "realloquer" ? "bg-blue-500/10" :
                        decision.actionType === "fatigue" ? "bg-yellow-500/10" :
                        "bg-purple-500/10"
                      )}>
                        <ActionIcon className={cn(
                          "h-4 w-4",
                          decision.actionType === "couper" ? "text-red-400" :
                          decision.actionType === "scaler" ? "text-emerald-400" :
                          decision.actionType === "realloquer" ? "text-blue-400" :
                          decision.actionType === "fatigue" ? "text-yellow-400" :
                          "text-purple-400"
                        )} />
                      </div>
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1 flex-wrap">
                          <Badge variant={getActionBadgeVariant(decision.actionType)}>
                            {getActionLabel(decision.actionType)}
                          </Badge>
                          <span className="text-sm font-semibold text-text-primary">{decision.creativeName}</span>
                          <Badge variant={getDecisionStatusBadge(decision.status)}>
                    ...
                    {decision.status === "en_attente" && (
                      <div className="flex gap-1.5 shrink-0">
                        <Button size="sm" variant="ghost" onClick={() => handleApplyDecision(decision.id)} className="h-7 px-2 text-emerald-400 hover:text-emerald-300">
                          <CheckCircle className="h-3.5 w-3.5" />
                        </Button>
                        <Button size="sm" variant="ghost" onClick={() => handleCancelDecision(decision.id)} className="h-7 px-2 text-red-400 hover:text-red-300">
...
function CycleCreatifTab() {
  ...
  const [generatedCreatives, setGeneratedCreatives] = useState<string | null>(null);
  ...
  const handleGenerateCreatives = async () => {
    setGenerating(true);
    try {
      const winnerInfo = winners.map((w) => `- ${w.name}: ROAS ${w.roas}x, CTR ${w.ctr}%, CPA ${w.cpa}€`).join("\n");
      const patternInfo = patterns.map((p) => `- ${p.element} (${p.category}, score: ${p.score}/100)`).join("\n");
...
Génère des variations innovantes qui reprennent ces patterns gagnants avec de nouveaux angles et hooks. Format: pour chaque creative, donne un nom, le hook, le body copy, le CTA, et l'angle utilisé.`,
      ...
      if (res.ok) {
        ...
        if (reader) {
          ...
          setGeneratedCreatives(data.content || data.text || JSON.stringify(data));
        }
              ...
              <div key={cycle.week} className="flex items-center shrink-0">
                ...
                >
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-xs font-semibold text-text-primary">Semaine {cycle.week}</span>
                    <Badge
                      variant={cycle.status === "en_cours" ? "default" : cycle.status === "termine" ? "default" : "muted"}
                      className="text-[9px] px-1.5"
                    >
                      {cycle.status === "en_cours" ? "En cours" : cycle.status === "termine" ? "Terminé" : "Planifié"}
                    </Badge>
                  </div>
                  <p className="text-[10px] text-text-muted">{cycle.startDate} → {cycle.endDate}</p>
                  <div className="flex items-center gap-3 mt-2 text-[10px]">
                    <span className="text-emerald-400">{cycle.winnersCount} gagnantes</span>
                    <span className="text-red-400">{cycle.losersCount} coupées</span>
                    <span className="text-blue-400">{cycle.newCreatives} nouvelles</span>
                  </div>
                </div>
                {i < cycles.length - 1 && <ChevronRight className="h-4 w-4 text-text-muted mx-1 shrink-0" />}
              </div>
            ))}
          </div>
        </CardContent>
      ...
      <Card>
        ...
        <CardContent>
          {winners.length > 0 ? (
              ...
              {winners.map((w) => (
                <div key={w.id} className="flex items-center justify-between p-3 rounded-xl bg-emerald-500/5 border border-emerald-500/20">
                  <div>
                    ...
                    <p className="text-[10px] text-text-muted">{w.campaignName}</p>
                  </div>
                  <div className="flex items-center gap-4 text-xs">
                    <span className="text-emerald-400 font-semibold">ROAS {w.roas}x</span>
            ...
            <p className="text-sm text-text-muted text-center py-4">Aucune creative gagnante cette semaine.</p>
          )}
        </CardContent>
      </Card>
        ...
        <CardContent>
          <div className="space-y-2">
            {patterns.map((pattern) => (
              <div key={pattern.element} className="flex items-center justify-between p-3 rounded-xl bg-bg-tertiary border border-border-default">
                <div className="flex items-center gap-3">
                  <Badge variant="muted" className="text-[10px]">{pattern.category}</Badge>
                  <span className="text-sm text-text-primary">{pattern.element}</span>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-semibold text-accent w-8 text-right">{pattern.score}%</span>
                </div>
              </div>
            ))}
          </div>
          ...
          {generatedCreatives ? (
            <div>
              ...
              <div className="flex gap-2 mt-4">
                <Button size="sm" variant="ghost" onClick={handleGenerateCreatives} disabled={generating}>
                  <RefreshCw className={cn("h-3.5 w-3.5 mr-1", generating && "animate-spin")} />
                  Regénérer
                </Button>
                <Button size="sm" variant="ghost" onClick={() => setGeneratedCreatives(null)}>
                  Fermer
                </Button>
              </div>
            </div>
            ...
            <div className="text-center py-6">
              <p className="text-sm text-text-muted mb-4">
                Génère de nouvelles variations basées sur les patterns gagnants de tes meilleures créatives.
              </p>
...
function ScalingTab() {
  ...
  const rollbackIndex = budgetHistory.findIndex((h, i) => i > 0 && h.budget < budgetHistory[i - 1].budget);
    ...
    <div className="space-y-6">
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-accent">{currentBudget}€<span className="text-sm text-text-muted">/jour</span></p>
              </div>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-emerald-400">{currentRoas}x</p>
              </div>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-lg font-bold text-text-primary">{currentTier.label}</p>
              </div>
                ...
                >
                  <div className="flex items-center justify-between flex-wrap gap-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className={cn("text-sm font-semibold", isCurrent ? "text-accent" : "text-text-primary")}>{tier.label}</p>
                        <p className="text-[10px] text-text-muted">
                          Budget : {tier.budgetMin}€ — {tier.budgetMax === 99999 ? "\u221E" : `${tier.budgetMax}€`}/jour
                        </p>
                      </div>
                    </div>
                    <div className="flex items-center gap-4 text-right">
                      ...
                      <div>
                        ...
                        <p className={cn(
                          ...
                          isCurrent && meetsRoas ? "text-emerald-400" :
                          isCurrent && !meetsRoas ? "text-red-400" :
                          "text-text-primary"
                        )}>
                          {tier.roasRequired}x
                        </p>
                  ...
                  {isCurrent && meetsRoas && (
                    <div className="mt-3 p-3 rounded-lg bg-accent/5 border border-accent/20">
                      <p className="text-xs text-accent flex items-center gap-1.5">
                        ...
                        Recommandation : augmenter le budget de {currentBudget}€ à {Math.round(currentBudget * (1 + tier.scalePercent / 100))}€/jour (+{tier.scalePercent}%)
                      </p>
                    </div>
                  )}
                </div>
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-3">
            {[
              { rule: "Si le ROAS baisse de plus de 30% en 48h", action: "Rollback au budget précédent", severity: "red" },
              { rule: "Si le CPA dépasse 2x le seuil pendant 24h", action: "Réduction de 25% du budget", severity: "yellow" },
              { rule: "Si le CTR passe sous 0.5% après un scale", action: "Pause et analyse avant reprise", severity: "yellow" },
              { rule: "Si les dépenses dépassent le plafond journalier de 10%", action: "Alerte + cap automatique", severity: "blue" },
            ].map((item) => (
              <div key={item.rule} className="flex items-start gap-3 p-3 rounded-xl bg-bg-tertiary border border-border-default">
                <ShieldAlert className={cn(
                  "h-4 w-4 mt-0.5 shrink-0",
                  item.severity === "red" ? "text-red-400" :
                  item.severity === "yellow" ? "text-yellow-400" :
                  "text-blue-400"
                )} />
                <div>
          ...
          {rollbackIndex > 0 && (
            <div className="mt-4 p-3 rounded-xl bg-purple-500/10 border border-purple-500/20">
              <p className="text-xs text-purple-400 flex items-center gap-1.5">
                ...
                Dernier rollback : {budgetHistory[rollbackIndex].date} — Budget réduit de {budgetHistory[rollbackIndex - 1].budget}€ à {budgetHistory[rollbackIndex].budget}€ (ROAS tombé à {budgetHistory[rollbackIndex].roas}x)
              </p>
            </div>
          )}
        </CardContent>
      ...
      <Card>
        ...
        <CardContent>
          <div className="h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={budgetHistory}>
                ...
                />
                <ReferenceLine yAxisId="budget" y={50} stroke="#A78BFA" strokeDasharray="3 3" label={{ value: "Palier 2", position: "right", fill: "#A78BFA", fontSize: 10 }} />
                <Area
                  yAxisId="budget"
                  type="monotone"
                  dataKey="budget"
          ...
          <div className="flex items-center justify-center gap-6 mt-3 text-xs text-text-muted">
            ...
            <span className="flex items-center gap-1.5">
              <span className="w-3 h-0.5 bg-purple-400 rounded" /> Seuil palier 2
            </span>
          </div>
        </CardContent>
      </Card>
