...
export function FollowerAdsGenerator({ initialData }: Props) {
  ...
  if (loading) {
    return <AILoading variant="immersive" text="Création de tes Follower Ads" />;
  }
      ...
      <div className="max-w-xl mx-auto py-8">
        <Card>
          ...
          <CardContent className="space-y-5">
            {/* Platform */}
            <div>
              <label className="text-sm font-medium text-text-primary mb-1.5 block">Plateforme</label>
              <p className="text-xs text-text-muted mb-2">La plateforme où tu veux gagner des abonnés</p>
            ...
            <div>
              ...
              <p className="text-xs text-text-muted mb-2">L&apos;objectif principal de cette campagne</p>
              <div className="flex flex-wrap gap-2">
                {OBJECTIVES.map((obj) => (
                  <button
                    key={obj}
            ...
            <GenerateButton onClick={generate} className="w-full" icon={<Users className="h-4 w-4 mr-2" />}>
      ...
      {data.reels_ads && data.reels_ads.length > 0 && (
        <section>
          <div className="flex items-center gap-2 mb-3">
            <h4 className="text-sm font-medium text-accent">Reels Ads (15-30s)</h4>
            <Badge variant="purple" className="text-[10px]">{data.reels_ads.length} scripts</Badge>
          </div>
                ...
                <div className="flex items-start justify-between gap-2">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-[10px] px-2 py-0.5 rounded-full bg-purple-500/10 text-purple-400 font-medium">Reel</span>
                    ...
                    <p className="text-[10px] text-text-muted mt-2">Audience : {ad.target_audience}</p>
                  </div>
                  <button
      ...
      {data.image_ads && data.image_ads.length > 0 && (
        <section>
          <div className="flex items-center gap-2 mb-3">
            ...
            <Badge variant="blue" className="text-[10px]">{data.image_ads.length} visuels</Badge>
          </div>
            ...
            {data.image_ads.map((ad, i) => (
              <div key={i} className="rounded-xl border border-border-default bg-bg-secondary hover:border-border-hover p-4 group transition-all duration-200">
                ...
                <div className="p-2 rounded-lg bg-accent/10 border border-accent/20 mt-2">
                  <p className="text-xs text-accent font-medium text-center">{ad.cta}</p>
                </div>
      ...
      {data.carousel_ads && data.carousel_ads.length > 0 && (
        <section>
          <div className="flex items-center gap-2 mb-3">
            ...
            <Badge variant="cyan" className="text-[10px]">{data.carousel_ads.length} carousels</Badge>
          </div>
          {data.carousel_ads.map((carousel, i) => (
            <div key={i} className="rounded-xl border border-border-default bg-bg-secondary hover:border-border-hover p-4 mb-3 transition-all duration-200">
              <p className="text-sm font-semibold text-text-primary mb-2">{carousel.title}</p>
              <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-thin">
                {carousel.slides.map((slide) => (
                  <div key={slide.slide_number} className="min-w-[140px] rounded-xl bg-bg-tertiary border border-border-default p-3 flex-shrink-0">
              ...
              <div className="p-2 rounded-lg bg-accent/10 border border-accent/20 mt-2 inline-block">
                <p className="text-xs text-accent font-medium">{carousel.final_cta}</p>
              </div>
            </div>
          ))}
        </section>
      ...
      {data.targeting_suggestions && (
        <section>
          ...
          <div className="rounded-xl border border-border-default bg-bg-secondary p-4 grid gap-3 md:grid-cols-2">
            <div>
              <p className="text-xs text-text-muted mb-1 font-medium">Intérêts</p>
              <div className="flex flex-wrap gap-1">
                {data.targeting_suggestions.interests.map((interest, idx) => (
                  <span key={idx} className="text-[10px] px-2 py-0.5 rounded-full bg-accent/10 text-accent">{interest}</span>
                ))}
              </div>
            </div>
            <div>
              <p className="text-xs text-text-muted mb-1 font-medium">Lookalike</p>
              <p className="text-xs text-text-primary">{data.targeting_suggestions.lookalike_source}</p>
            </div>
            <div>
              <p className="text-xs text-text-muted mb-1 font-medium">Budget / jour</p>
              <p className="text-xs text-text-primary font-semibold">{data.targeting_suggestions.budget_daily}</p>
            </div>
            <div>
              <p className="text-xs text-text-muted mb-1 font-medium">Durée test</p>
