...
export function CarouselGenerator({
  ...
  const [collapsedCarousels, setCollapsedCarousels] = React.useState<
    Record<number, boolean>
  >({});
  const [expandedCarousel, setExpandedCarousel] = React.useState<number | null>(
    null,
  );
  ...
  const handleMassiveGenerate = async () => {
    ...
    for (let batchIdx = 0; batchIdx < totalBatches; batchIdx++) {
      ...
      const batchTopics = topics.slice(
        batchIdx * topicsPerBatch,
        (batchIdx + 1) * topicsPerBatch,
      );
      ...
      <div className={cn("space-y-4", className)}>
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Crée des carousels Instagram/LinkedIn optimisés pour les saves et
              partages.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-5">
            <div>
              ...
              <Input
                ...
                onKeyDown={(e) =>
                  e.key === "Enter" && !massiveMode && handleGenerate()
                }
              />
            </div>
            ...
            <div className="p-4 rounded-xl bg-bg-tertiary/50 border border-border-default">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                >
                  ...
                  <p className="text-xs text-text-muted">
                    10+ sujets variés = 40+ carousels prêts à poster
                  </p>
                </button>
              </div>
            </div>
            ...
            {massiveMode && (
              <div className="space-y-1.5">
                <p className="text-xs font-medium text-text-primary">
                  Sujets qui seront générés :
                </p>
                <div className="grid grid-cols-2 gap-1.5">
                  {CAROUSEL_TOPICS.map((t, i) => (
                    <div
                      key={i}
                      className="flex items-center gap-2 p-1.5 rounded-lg bg-bg-tertiary/30"
                    >
                ...
                {topic.trim() && (
                  <p className="text-[10px] text-accent">
                    + ton sujet personnalisé : &quot;{topic.trim()}&quot;
                  </p>
                )}
              </div>
            )}
            ...
            <p className="text-sm text-text-secondary text-center">
              {massiveMode
                ? "Génération massive — plusieurs minutes de traitement"
                : "Carousel 8-10 slides avec cover, contenu et CTA"}
            </p>
          </CardContent>
        </Card>
        ...
        <GlowCard glowColor="orange">
          <div className="flex items-center justify-between mb-2">
            ...
            <Button
              ...
              size="sm"
              onClick={() =>
                copyToClipboard(result.hook_cover, `${prefix}-cover`)
              }
            >
              {copiedField === `${prefix}-cover` ? (
          ...
          <CardContent className="pt-5">
            <div className="flex items-center justify-between mb-2">
              ...
              <Button
                ...
                size="sm"
                onClick={() =>
                  copyToClipboard(result.caption, `${prefix}-caption`)
                }
              >
                {copiedField === `${prefix}-caption` ? (
                  <Check className="h-3 w-3 mr-1 animate-in zoom-in-50 duration-200" />
                ) : (
                  <Copy className="h-3 w-3 mr-1" />
                )}
        ...
        <div className="flex flex-wrap gap-1">
          {result.hashtags.map((h, i) => (
            <Badge
              ...
              onClick={() => copyToClipboard(h, `${prefix}-hash-${i}`)}
            >
