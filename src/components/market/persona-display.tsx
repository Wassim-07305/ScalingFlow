...
const PERSONA_TABS = [
  ...
  { key: "icp", label: "ICP Pro Max", icon: Sparkles },
...
function buildPersonaPDFContent(persona: PersonaForgeResult): string {
  ...
  if (persona.based_on_real_data) {
    text += `=== SOURCE : DONNÉES YOUTUBE RÉELLES ===\n\n`;
  }

  if (persona.verbatims_reels?.length) {
    text += `=== VERBATIMS RÉELS ===\n${persona.verbatims_reels.map((v) => `« ${v} »`).join("\n")}\n\n`;
  }

  if (persona.niveau_schwartz) {
    text += `=== NIVEAU SCHWARTZ ===\n${persona.niveau_schwartz}\n\n`;
  }

  if (persona.raisons_achat?.length) {
    text += `=== RAISONS D'ACHAT ===\n${persona.raisons_achat.join("\n")}\n\n`;
  }

  if (persona.desirs_par_profondeur) {
    text += `=== DÉSIRS PAR PROFONDEUR ===\n`;
    if (persona.desirs_par_profondeur.surface?.length) {
      text += `Surface : ${persona.desirs_par_profondeur.surface.join(", ")}\n`;
    }
    if (persona.desirs_par_profondeur.intermediaire?.length) {
      text += `Intermédiaire : ${persona.desirs_par_profondeur.intermediaire.join(", ")}\n`;
    }
    if (persona.desirs_par_profondeur.profond?.length) {
      text += `Profond : ${persona.desirs_par_profondeur.profond.join(", ")}\n`;
    }
    text += "\n";
  }
...
function TagList({
  ...
  items: string[];
  variant?: "default" | "red" | "yellow" | "blue" | "purple" | "muted";
}) {
  return (
...
export function PersonaDisplay({ persona }: PersonaDisplayProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* ICP Pro Max : Verbatims, Schwartz, Raisons d'achat, Désirs par profondeur */}
      {activeTab === "icp" && (
        <div className="space-y-4">
          {/* Verbatims réels */}
          {persona.verbatims_reels && persona.verbatims_reels.length > 0 && (
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
              ...
              <CardContent className="space-y-2">
                {persona.verbatims_reels.map((v, i) => (
                  <p
                    key={i}
                    className="text-sm text-text-primary italic border-l-2 border-accent/40 pl-3 py-1"
                  >
                    &laquo; {v} &raquo;
                  </p>
                ))}
          ...
          {/* Raisons d'achat */}
          {persona.raisons_achat && persona.raisons_achat.length > 0 && (
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
          ...
          {/* Désirs par profondeur */}
          {persona.desirs_par_profondeur && (
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
              ...
              <CardContent className="space-y-4">
                {persona.desirs_par_profondeur.surface &&
                  persona.desirs_par_profondeur.surface.length > 0 && (
                    <SectionBlock title="Surface (ce qu'il dit vouloir)">
                      <TagList
                        items={persona.desirs_par_profondeur.surface}
                        variant="blue"
                      />
                    </SectionBlock>
                  )}
                {persona.desirs_par_profondeur.intermediaire &&
                  persona.desirs_par_profondeur.intermediaire.length > 0 && (
                    <SectionBlock title="Intermédiaire (ce qu'il veut vraiment)">
                      <TagList
                        items={persona.desirs_par_profondeur.intermediaire}
                        variant="yellow"
                      />
                    </SectionBlock>
                  )}
                {persona.desirs_par_profondeur.profond &&
                  persona.desirs_par_profondeur.profond.length > 0 && (
                    <SectionBlock title="Profond (besoin fondamental caché)">
                      <TagList
                        items={persona.desirs_par_profondeur.profond}
                        variant="purple"
                      />
                    </SectionBlock>
                  )}
      ...
      {activeTab === "demo" && persona.niveau_1_demo && (
        <Card>
          ...
          <CardContent className="divide-y divide-border-default">
            <InfoRow
              label="Tranche d'âge"
              value={persona.niveau_1_demo.age_range}
            />
            <InfoRow label="Genre" value={persona.niveau_1_demo.genre} />
            <InfoRow
              label="Situation familiale"
              value={persona.niveau_1_demo.situation_familiale}
            />
            <InfoRow
              label="Revenu annuel"
              value={persona.niveau_1_demo.revenu_annuel}
            />
            <InfoRow
              label="Localisation"
              value={persona.niveau_1_demo.localisation}
            />
            <InfoRow
              label="Situation pro"
              value={persona.niveau_1_demo.situation_pro || "—"}
            />
            <InfoRow
              label="Éducation"
              value={persona.niveau_1_demo.niveau_education}
            />
          </CardContent>
        </Card>
      )}
        ...
        <Card>
          ...
          <CardContent className="space-y-5">
            <SectionBlock title="Peurs" icon={ShieldAlert}>
              <TagList
                items={persona.niveau_2_psycho.peurs ?? []}
                variant="red"
              />
            </SectionBlock>
            <SectionBlock title="Frustrations">
              <TagList
                items={persona.niveau_2_psycho.frustrations ?? []}
                variant="yellow"
              />
            </SectionBlock>
            <SectionBlock title="Désirs profonds">
              <TagList
                items={persona.niveau_2_psycho.desirs_profonds ?? []}
                variant="default"
              />
            </SectionBlock>
            <SectionBlock title="Croyances limitantes">
              <TagList
                items={persona.niveau_2_psycho.croyances_limitantes ?? []}
                variant="muted"
              />
            </SectionBlock>
            <SectionBlock title="Déclencheurs d'achat" icon={Zap}>
              <TagList
                items={persona.niveau_2_psycho.declencheurs_achat ?? []}
                variant="purple"
              />
            </SectionBlock>
          </CardContent>
        ...
        <Card>
          ...
          <CardContent className="space-y-5">
            <SectionBlock title="Habitudes digitales" icon={Globe}>
              <div className="space-y-1.5">
                {(
                  persona.niveau_3_comportemental.habitudes_digitales ?? []
                ).map((h, i) => (
                  <div key={i} className="flex items-start gap-2">
                    <span className="mt-1.5 h-1.5 w-1.5 rounded-full bg-accent shrink-0" />
            ...
            <SectionBlock title="Réseaux sociaux utilisés">
              <TagList
                items={persona.niveau_3_comportemental.reseaux_sociaux ?? []}
                variant="blue"
              />
            </SectionBlock>
            <SectionBlock title="Type de contenu consommé">
              <TagList
                items={
                  persona.niveau_3_comportemental.type_contenu_consomme ?? []
                }
                variant="default"
              />
            </SectionBlock>
            <SectionBlock title="Objections typiques" icon={MessageSquare}>
              <div className="space-y-1.5">
                {(
                  persona.niveau_3_comportemental.objections_typiques ?? []
                ).map((obj, i) => (
            ...
            <SectionBlock title="Appareils utilisés" icon={Smartphone}>
              <TagList
                items={persona.niveau_3_comportemental.appareils_utilises ?? []}
                variant="muted"
              />
            </SectionBlock>
          </CardContent>
        ...
        <Card>
          ...
          <CardContent className="space-y-5">
            <SectionBlock title="Parcours d'achat" icon={Route}>
              <div className="space-y-2">
                {(persona.niveau_4_strategique.parcours_achat ?? []).map(
                  (etape, i) => (
            ...
            <SectionBlock title="Points de contact optimaux" icon={BarChart3}>
              <TagList
                items={
                  persona.niveau_4_strategique.points_contact_optimaux ?? []
                }
                variant="blue"
              />
            </SectionBlock>
            <SectionBlock title="Messages qui résonnent">
              <div className="space-y-1.5">
                {(
                  persona.niveau_4_strategique.messages_qui_resonnent ?? []
                ).map((msg, i) => (
            ...
            <SectionBlock title="Critères de décision">
              <TagList
                items={persona.niveau_4_strategique.criteres_decision ?? []}
                variant="purple"
              />
            </SectionBlock>
            <SectionBlock title="Influenceurs / Prescripteurs">
              <TagList
                items={
                  persona.niveau_4_strategique.influenceurs_prescripteurs ?? []
                }
                variant="muted"
              />
            </SectionBlock>
