...
export function PainIdentifier({
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      setPains(data);
      toast.success("Bleeding-neck pains identifies !");
    } catch (err) {
      toast.error(
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-text-muted">
              {pains.total_pains} pains identifies
            </p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <Badge variant={pains.severity_score >= 70 ? "red" : "yellow"}>
            Severite : {pains.severity_score}/100
          </Badge>
